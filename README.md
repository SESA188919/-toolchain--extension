 ...
 * Author  : Yves Teissier
 * Contact : yteissier@assystem.com 
 */

 Please replace the key world $toolchain$ and $TOOLCHAIN$ by the appropriate toolchain name.
 
$toolchain$-extension
=============

 This software is a plugin enabling $TOOLCHAIN$ compiler and linker support for the
 Mindc toolchain (http://mind.ow2.org). It enables the use of $TOOLCHAIN$-specific
...
- 	Une classe $TOOLCHAIN$CompilerWrapper qui étend le GCCCompilerWrapper et qui doit
	être complétée pour adapter la ligne de commande générée pour exécuter le
	préprocesseur, le compilateur et le linker.
- 	Une classe $TOOLCHAIN$CompilerModule. La toolchain MIND utilise un outil
	d’injection de dépendance nommé ‘Google Guice’. Cette classe
	$TOOLCHAIN$CompilerModule est un module Guice qui spécifie que l’interface
	« CompilerWrapper » est implémentée par la class « $TOOLCHAIN$CompilerWrapper ».
...
- 	Un fichier ‘mind-plugin.xml’. C’est le descripteur de l’extension. Le
	...
	o   La première ajoute le flag ‘—$toolchain$‘ sur la ligne de commande ‘mindc’.
		Avec cette extension, ‘mindc’ peut reconnaitre le flag ‘’$toolchain$’ sur la
		ligne de commande.
	o   La deuxième ajoute le module Guice « $TOOLCHAIN$CompilerModule ». Ce module
		...
		active que si le flag ‘—$toolchain$’ est présent sur la ligne de commande de
		mindc. Ceci permet d’utiliser le $TOOLCHAIN$CompilerWrapper si et seulement si
		le flag ‘-$toolchain$’ est spécifié sur la ligne de commande de mindc.
...
il faut lancer mindc avec l’option ‘—$toolchain$’.